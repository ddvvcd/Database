...
`t_no`			TINYINT PRIMARY KEY NOT NULL AUTO_INCREMENT,
...
SELECT * FROM `bank_account` 
WHERE `a_balance` >= 1000000 
AND `a_item_name` = '자유저축예금';
...
#실습 2-23 (GROUP BY : 계좌번호로 조회) (ORDER BY로 `t_a_no` 정렬)
...
#실습 2-24 (위에 적은 `합계` 를 HAVING 으로 가져와서 입력!)
SELECT
	`t_a_no`,
	`t_dist`,
	SUM(`t_amount`) AS `합계`
